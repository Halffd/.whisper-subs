"""
WhisperSubs GTK4 GUI - Desktop Application
...
class TranscriptionApp(Gtk.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        self._model_items = get_flat_display_list()
    ...
    def setup_ui(self):
        ...
        # Model selection - provider-grouped with separators
        ...
        factory = self.model_combo.get_factory()
        if factory is None:
            self.model_combo.set_factory(Gtk.SignalListItemFactory())
            factory = self.model_combo.get_factory()
    ...
    def _on_factory_bind(self, factory, list_item):
        ...
        text = string_obj.get_string() if string_obj else ""
        label.set_text(text)
        if _is_sep(text):
    ...
    def _get_selected_model(self):
        pos = self.model_combo.get_selected()
        if pos < len(self._model_items):
            item = self._model_items[pos]
            if not _is_sep(item):
                return item
