...
def transcribe_audio(audio_file, model_name, language = None, device = 'cuda', compute = 'int8_float32'):
    ...
    try:
        ...
        print(e, file=sys.stderr)
    return None
def process_create(file, model_name, language = 'none', device = 'cuda', compute = 'int8_float32'):
        ...
        for c in range(0, 3):
            ...
            for j in range(i, -1, -1):
                try:
                    ...
                    print(model_name)
                    print(f"Starting subprocess with model: {model_name}")
                        ...
                        if exit_code == 0:
                            ...
                            try:
                                with open('segments.json', 'r') as file:
                                    ...
                                    print("Data successfully loaded from segments.json")
                                    print(loaded_segments)  # Print the loaded segments
                                    return convert_dict_to_segments(loaded_segments)
                            except (IOError, json.JSONDecodeError) as e:
                                print(f"Error reading from file: {e}")
                        else:
                            print(f"Process exited with code: {exit_code}. Error: {stderr}")
                    except Exception as e:
                        print(f"An error occurred while handling: {e}")
                except Exception as e:
                    print(f"An error occurred on creation: {e}")
    else:
        print('No model')
